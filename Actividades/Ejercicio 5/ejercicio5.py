...
class PlanAhorro:
    ...
    def __init__(self, cod, mod, ver, valor, cuo, cantmin):
        self.__cod = cod
        self.__modelo = mod
        self.__version = ver
        self.__valor = valor
        self.__cuotas = cuo
        self.__CantCuotasMin = cantmin

    def __str__ (self):
        return (f"{self.__cod},{self.__modelo},{self.__version}")
...
class ManejadorPlan:
    ...
    def actualizaValorPlan(self):
        ...
        while not b and i in range(len(self.__lista)):
            codigo = input('Ingrese codigo de plan para modificar el valor del vehiculo: ')
    ...
    def mostrarMontos(self):
        ...
        while (i<len(self.__lista)):
            ...
            if(codigo==self.__lista[i].getCod()):
                print(f'El monto que se debe haber pagado para licitar el vehiculo es: {self.__lista[i].getCantCuotasMIn() * self.__lista[i].valorcuota}')
            else:
                i=i+1
        return
